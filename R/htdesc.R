...
# possibly can use stringr and use '\\\\' for regex match and change, but note that this is not a real problem, since the inputcsvfiles will have smiles which are already forced with double \\ characters by R itself when it read it
  ...
  if (sigma.selection =="A") {
    ...
    returnlist <- list (tanimoto = fmcsoutputframe$Tanimoto_Coefficient[1], index = fmcsoutputframe$index[1], sub = fmcsoutputframe$fragments[1], value = fmcsoutputframe$B[1])
    return(returnlist)
  } else if (sigma.selection == "C") {
    #C: Hydrowin 1st option plus A (duplicate value of highest occurance aka mode )
    returnlist <- list (tanimoto = fmcsoutputframe$Tanimoto_Coefficient[1], index = fmcsoutputframe$index[1], sub = fmcsoutputframe$fragments[1], value = fmcsoutputframe$C[1])
    return(returnlist)
